...
CSV_URL = "https://ons-aws-prod-opendata.s3.amazonaws.com/dataset/disponibilidade_usina_ho/DISPONIBILIDADE_USINA_2025_08.csv"
SNOWFLAKE_CONN_ID = "snowflake_dev"
TARGET_DATABASE = None  # usa o database do connection
TARGET_SCHEMA = "STAGING"
TARGET_TABLE = "DISPONIBILIDADE_USINA_2025_08"


@dag(
    dag_id="import_disponibilidade_usina_2025_08",
...
def import_disponibilidade():
    ...
    def download_csv(url: str) -> str:
        ...
        file_path = os.path.join(tmp_dir, TARGET_TABLE + ".csv")
    ...
    def create_table_if_not_exists():
        """Cria a tabela destino com tipos razoáveis caso ainda não exista."""
    ...
    # Normaliza nomes de colunas: remove BOM, espaços, e caracteres problemáticos
        ...
        cleaned_file = os.path.join(os.path.dirname(file_path), TARGET_TABLE + "_cleaned.csv")
        try:
        ...
        # Usar PUT + COPY INTO no stage da tabela para evitar problemas com identifiers
        # PUT envia o arquivo local para o stage da tabela (@%<table>)
        file_uri = f"file://{os.path.abspath(cleaned_file)}"
        try:
            cur.execute(f"PUT '{file_uri}' @%{TARGET_TABLE} OVERWRITE=TRUE;")
        except Exception as e:
            raise AirflowFailException(f"Erro ao dar PUT no stage da tabela: {e}")

        # listar arquivos no stage (útil para debug)
        try:
            cur.execute(f"LIST @%{TARGET_TABLE};")
            staged = cur.fetchall()
    ...
    path = download_csv(CSV_URL)
    create = create_table_if_not_exists()
    load = load_to_snowflake(path)
